...
当前主功能是戴森球计划量化计算；页面已加入异星工厂初版入口，用于逐步接入 Factorio 数据导入、配方/建筑选择和通用求解流程。现阶段异星工厂页面仍是接入骨架，不等同于完整可用的 Factorio 计算器。

戴森球计划计算结果支持导出为 xlsx 表格，导出内容包含目标产物、建筑统计、原矿化/原料统计、预估电力和明细行；图标图片不会写入文件，物品、建筑和增产剂图标会用对应名称替代。结果表也支持导出 DSP 蓝图 `.txt` 文件，文件内容是游戏蓝图字符串，可放入 `Dyson Sphere Program\Blueprint` 目录或通过游戏蓝图导入入口使用。当前蓝图第一版写入生产建筑、配方、建筑模型和蓝图校验信息，暂未生成传送带、分拣器、物流塔和喷涂机布局。
